...
- **Resume picks up where it left off.** A **Resume** button appears on the Roomba tab whenever a sweep stopped with work outstanding. It carries on from that sweep's queue and skips the scan entirely — in a large gang house the scan is most of the time a sweep takes, and stopping early doesn't make what it already found wrong. Items someone else has taken in the meantime simply fail their pickup and drop out of the queue, exactly as they would mid-sweep. Use **Start Sweep** instead when you want a fresh look at every room.
