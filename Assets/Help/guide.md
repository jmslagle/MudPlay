...
- **An item Roomba thinks it's carrying but isn't:** if a drop comes back with the game's currency-syntax complaint (`Syntax: DROP {Amount} {Currency}`), the game doesn't recognise the name as something you're holding. Roomba reads your inventory to check rather than assuming: if the item genuinely isn't there, the move is discarded, since a drop for something you don't have can only ever fail again; if it *is* there, the move stays queued and is retried later.
